...
import { CombatLog, formatAmount, formatCombatTime, formatEssenceGrade } from "../combat/CombatLog";
  ...
  return (
    <section className="screen-panel hunt-screen">
      ...
      <article className="panel combat-record-panel">
          ...
          <CombatLog
            logs={result ? visibleLogs : []}
            playerName={result?.player.name ?? character.name}
            enemyName={result?.enemy.name ?? "적"}
            enemyLevel={result?.enemy.level ?? 0}
            gainedExperience={result?.gainedExperience}
            gainedCredits={result?.gainedCredits ?? result?.rewards?.credits}
            emptyMessage={hasEncounteredMonster ? "조우 완료. 전투를 시작하세요." : result ? "전투 개시 중..." : "몬스터 찾기를 기다리고 있습니다."}
            listRef={logRef}
  ...
  return (
    <article className="panel last-battle-result-panel">
      ...
      <div className="hunt-result-summary">
        <Kv label="전투 시간" value={formatCombatTime(durationTicks)} />
