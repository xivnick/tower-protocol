create table public.character_training_states (
...
alter table public.character_training_states enable row level security;

create trigger character_training_states_set_updated_at
before update on public.character_training_states
for each row
...
begin
  ...
  elapsed_charges := greatest(0, floor(extract(epoch from (current_time - training_state.last_recharged_at)) / 6)::integer);
...
begin
  ...
  if calculated_charges = 10 then recharged_at := current_time; end if;
