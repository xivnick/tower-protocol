...
create or replace function public.combat_total_defense(combat_stats jsonb)
returns numeric
language sql
immutable
as $$
  select coalesce((combat_stats ->> 'physical_defense')::numeric, 0)
       + coalesce((combat_stats ->> 'magic_defense')::numeric, 0);
$$;
...
begin
  ...
  combat_stats := public.monster_combat_stats_at_level(dummy_template, target_character.level);
  return combat_stats || jsonb_build_object('name', dummy_template.name, 'level', target_character.level);
end;
$$;
