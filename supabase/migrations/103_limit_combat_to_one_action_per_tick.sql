...
begin
  ...
  if position(E'  player_used_essence_action boolean := false;\n  enemy_used_essence_action boolean := false;\n  index integer;' in definition) = 0
    or position(E'    player_used_essence_action := false;\n    enemy_used_essence_action := false;\n    for index in 1..player_essence_count loop' in definition) = 0
    or position(E'    if duration_ticks > 0 and not enemy_used_essence_action and enemy_essence_code is not null and enemy_essence_ready <= duration_ticks then' in definition) = 0
    or position(E'    if player_attack_gauge >= 1 and not player_used_essence_action then' in definition) = 0
    or position(E'      if monster_attack_gauge >= 1 and not enemy_used_essence_action then' in definition) = 0 then
    raise exception 'hunt_training_dummy_action_slot_definition_changed';
  end if;

  definition := replace(
    definition,
    E'  player_used_essence_action boolean := false;\n  enemy_used_essence_action boolean := false;\n  index integer;',
