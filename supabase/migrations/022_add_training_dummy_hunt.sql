...
add column if not exists hunt_available_at timestamptz;
...
begin
  ...
  if target_character.hunt_available_at is not null
    and target_character.hunt_available_at > clock_timestamp() then
  ...
  while duration_ticks < max_duration_ticks and dummy_hp > 0 loop
    ...
    if attack_gauge >= 1 then
      ...
      raw_damage := 10 * (1 + (target_character.strength::numeric / 100));
