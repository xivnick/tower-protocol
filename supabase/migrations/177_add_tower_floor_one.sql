...
alter table public.character_tower_progress enable row level security;

drop trigger if exists character_tower_progress_set_updated_at on public.character_tower_progress;
create trigger character_tower_progress_set_updated_at
before update on public.character_tower_progress
for each row
execute function public.set_updated_at();

revoke all on table public.character_tower_progress from anon, authenticated;

insert into public.hunt_grounds (
  id, name, recommended_min_level, recommended_max_level, sort_order, is_enabled, level_mode
)
values ('tower-floor-1', '탑 1층', 1, 1, 1001, false, 'fixed')
...
select 'tower-floor-1', monster.id, 1, 1, 1, 1, true
from public.monster_templates monster
where monster.code = 'angry-boar'
  and not exists (
    select 1
    from public.hunt_ground_monsters existing
    where existing.hunt_ground_id = 'tower-floor-1'
...
begin
  ...
  set selected_hunt_ground_id = 'tower-floor-1',
      available_at = null,
      last_battle = null,
...
begin
  ...
  set highest_cleared_floor = case
        when final_status = 'victory' then greatest(highest_cleared_floor, 1)
...
revoke execute on function public.get_my_tower_state() from public, anon;
revoke execute on function public.challenge_tower_floor_one() from public, anon;
revoke execute on function public.settle_tower_floor_one() from public, anon;

grant execute on function public.get_my_tower_state() to authenticated;
grant execute on function public.challenge_tower_floor_one() to authenticated;
grant execute on function public.settle_tower_floor_one() to authenticated;
